...
class PINNLoss(nn.Module):
    ...
    def __init__(
        ...
        # FIX 1: Mass-flow data consistency loss (CRITICAL)
        lambda_mdot: float = 0.0,
        # FIX 3: Vertical acceleration data consistency loss (RECOMMENDED)
        lambda_az: float = 0.0,
        # FIX 4: Burn floor loss (OPTIONAL)
        lambda_burn: float = 0.0,
    ):
        super().__init__()
        ...
        self.lambda_mdot = lambda_mdot
        self.lambda_az = lambda_az
        self.lambda_burn = lambda_burn
    ...
    def mass_flow_data_loss(
        ...
        Forces the model to learn the correct scale of mass depletion by
        matching predicted mass flow rate to reference mass flow rate.
        
        L_mdot = mean((m_dot_pred - m_dot_ref)^2)
        ...
        # Extract mass
        m_pred = pred_state[..., 13:14]  # [batch, N, 1]
        m_true = true_state[..., 13:14]  # [batch, N, 1]
        
        # Time derivatives (mass flow rates)
        m_dot_pred = self._finite_difference(m_pred, t)  # [batch, N, 1]
        m_dot_true = self._finite_difference(m_true, t)  # [batch, N, 1]
        
        return torch.mean((m_dot_pred - m_dot_true) ** 2)
    
    def vertical_accel_data_loss(
        self,
        pred_state: torch.Tensor,
        true_state: torch.Tensor,
        t: torch.Tensor,
        ...
        Aligns predicted vertical acceleration with reference acceleration,
        helping to reduce v_z divergence.
        
        L_az = mean((dvz_dt_pred - dvz_dt_ref)^2)
        ...
        vz_pred = pred_state[..., 5:6]  # [batch, N, 1]
        vz_true = true_state[..., 5:6]  # [batch, N, 1]
        
        # Time derivatives (vertical accelerations)
        dvz_pred = self._finite_difference(vz_pred, t)  # [batch, N, 1]
        dvz_true = self._finite_difference(vz_true, t)  # [batch, N, 1]
        
        return torch.mean((dvz_pred - dvz_true) ** 2)
    ...
    def burn_floor_loss(
        ...
        Soft bias to prevent zero-burn collapse. Penalizes mass flow rates
        that are too close to zero during burn phase.
        
        L_burn = mean(ReLU(eps - |m_dot|))
        
        Args:
            pred_state: [batch, N, 14] or [N, 14]
            t: Time [batch, N, 1] or [N, 1]
            eps: Threshold for minimum mass flow rate (default 1e-3)
        ...
        # Mass flow rate
        m_dot = self._finite_difference(m_pred, t)  # [batch, N, 1]
        
        # Penalty for mass flow rates too close to zero
        violations = torch.relu(eps - torch.abs(m_dot))  # [batch, N, 1]
    ...
    def forward(
        ...
        # FIX 1: Mass-flow data consistency loss (CRITICAL)
        L_mdot = self.mass_flow_data_loss(pred_state, true_state, t)
        
        # FIX 3: Vertical acceleration data consistency loss (RECOMMENDED)
        L_az = self.vertical_accel_data_loss(pred_state, true_state, t)
        
        # FIX 4: Burn floor loss (OPTIONAL)
        L_burn = self.burn_floor_loss(pred_state, t)
        ...
        total_loss = (
            ...
            + self.lambda_mdot * L_mdot  # FIX 1: Mass-flow data consistency
            + self.lambda_az * L_az  # FIX 3: Vertical acceleration data consistency
            + self.lambda_burn * L_burn  # FIX 4: Burn floor loss
        )
        ...
        loss_dict = {
            ...
            "mdot": L_mdot,  # FIX 1: Mass-flow data consistency
            "az": L_az,  # FIX 3: Vertical acceleration data consistency
            "burn": L_burn,  # FIX 4: Burn floor loss
        }
