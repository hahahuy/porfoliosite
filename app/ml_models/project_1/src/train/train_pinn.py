...
def main():
    ...
    loss_params = {
        ...
        # FIX 1, 3, 4: New calibration losses
        "lambda_mdot": safe_float(loss_cfg.get("lambda_mdot"), 0.0),
        "lambda_az": safe_float(loss_cfg.get("lambda_az"), 0.0),
        "lambda_burn": safe_float(loss_cfg.get("lambda_burn"), 0.0),
    }
    ...
    # FIX 2: Physics loss warm-up schedule (ESSENTIAL)
    # Prevents physics residual from dominating too early, before mass scale is learned
    physics_warmup_enabled = loss_cfg.get("physics_warmup", {}).get("enabled", True)
    lambda_phys_init = safe_float(loss_cfg.get("lambda_phys"), 0.1)
    lambda_phys_final = safe_float(loss_cfg.get("lambda_phys_final", lambda_phys_init), lambda_phys_init)
    physics_warmup_ratio = safe_float(loss_cfg.get("physics_warmup", {}).get("warmup_ratio", 0.3), 0.3)
    
    if physics_warmup_enabled:
        print(f"Physics loss warm-up enabled: {lambda_phys_init} -> {lambda_phys_final} over {int(physics_warmup_ratio * 100)}% of training")
    else:
        # If warm-up disabled, use final value from start
        loss_fn.lambda_phys = lambda_phys_final
    ...
    for epoch in range(start_epoch, n_epochs):
        # FIX 2: Update physics loss warm-up schedule
        if physics_warmup_enabled:
            progress = epoch / n_epochs
            if progress < physics_warmup_ratio:
                # Warm-up phase: keep physics loss low
                loss_fn.lambda_phys = lambda_phys_init
            else:
                # Ramp phase: linearly increase to final value
                alpha = (progress - physics_warmup_ratio) / (1.0 - physics_warmup_ratio)
                loss_fn.lambda_phys = lambda_phys_init + alpha * (lambda_phys_final - lambda_phys_init)
